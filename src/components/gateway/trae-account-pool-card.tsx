...
const POOL_TILES = [
  { key: "available", label: "可路由", tone: "ok" as const },
  { key: "cooling", label: "冷却中", tone: "warn" as const },
  { key: "disabled", label: "会话失效", tone: "danger" as const },
  { key: "expired", label: "积分过期", tone: "warn" as const },
  { key: "zeroCredits", label: "零积分", tone: "muted" as const },
] as const;
    ...
    <Card className={cn("gap-0 py-0", className)}>
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
        ...
        <span className="text-xs text-muted-foreground">
          累计请求 {formatCount(status?.totalRequests ?? 0)}
        </span>
      </div>
