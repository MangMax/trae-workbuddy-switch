...
const DONATE_CHANNELS = [
  { key: "wechat", name: "微信支付", image: donateWechat },
  { key: "alipay", name: "支付宝", image: donateAlipay },
] as const;
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>支持 Buddy Switch</DialogTitle>
          <DialogDescription>
            Buddy Switch 免费开源。如果它帮到了你，可以扫码请作者喝杯饮料。
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-4">
          {DONATE_CHANNELS.map((channel) => (
            <figure key={channel.key} className="flex flex-col gap-2">
              ...
              <figcaption className="text-center text-sm font-medium">{channel.name}</figcaption>
            </figure>
          ))}
        </div>
