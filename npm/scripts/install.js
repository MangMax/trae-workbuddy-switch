...
//
// 平台分包（esbuild 模式）：二进制发布在独立 npm 包（@mangmax/buddy-switch-<platform>-<arch>），
// 主包声明为 optionalDependencies，安装时 npm 自动装好平台包，postinstall 只需复制——
// 不依赖 GitHub，国内镜像（npmmirror）也能稳定安装。
//
// 环境变量覆盖：
//   BUDDY_SWITCH_BINARY=<本地二进制路径>  本地开发/离线安装（直接复制，不联网）
const fs = require("fs");
...
// 主包与平台包同在 `@mangmax` scope 下（包名带 scope，目录名不带）。
const PLATFORM_PKG = `@mangmax/buddy-switch-${process.platform}-${process.arch}`;
...
async function main() {
  // 1) 本地二进制覆盖（开发/离线）
  if (process.env.BUDDY_SWITCH_BINARY) {
    const local = path.resolve(process.env.BUDDY_SWITCH_BINARY);
    if (fs.existsSync(local)) return copyFrom(local);
    return fail(`BUDDY_SWITCH_BINARY 指向的文件不存在: ${local}`);
  }

  // 2) 从平台包复制（node_modules/@mangmax/buddy-switch-<platform>-<arch>/bin/<file>）
  try {
