#!/usr/bin/env node
// CodeBuddy CLI apiKeyHelper for buddy-switch.
//
// 只把当前选中账号的 token（Bearer xxx）打印到 stdout，CodeBuddy CLI 会把它
// 作为 Authorization 头使用。当前选中账号由 buddy-switch 写入
...
const accountsFile =
  process.env.BUDDY_SWITCH_ACCOUNTS_FILE || path.join(os.homedir(), ".buddy-switch", "accounts.json");
const stateFile = path.join(rotateDir, "state.json");

function fail(message) {
  process.stderr.write(`buddy-switch: ${message}\n`);
  process.exit(1);
}
