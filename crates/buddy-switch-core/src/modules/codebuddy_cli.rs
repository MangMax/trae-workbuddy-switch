...
//! 两种模式都复用 buddy-switch 的 WorkBuddy 账号库，但保持独立的当前账号。
...
/// buddy-switch 的旧 helper，允许直接原地升级，并清理旧文件。
...
mod tests {
    ...
    fn helper_test_dir() -> PathBuf {
        std::env::temp_dir().join(format!("buddy-switch-codebuddy-helper-{}", uuid::Uuid::new_v4()))
    }
  ...
  "env": { "HTTPS_PROXY": "http://127.0.0.1:7890" }
    ...
    fn extracts_node_path_without_accepting_shell_noise() {
        // 该函数解析登录 shell 的 `command -v node` 输出（macOS 上为 zsh）。
        // 输入使用当前平台的绝对路径，保证 Windows 上也覆盖同一段逻辑。
    ...
    fn helper_selects_active_account_and_fails_without_selected_token() {
        ...
        let output = Command::new("node")
            ...
            .env("BUDDY_SWITCH_ACCOUNTS_FILE", &accounts_file)
            .output()
            .unwrap();
